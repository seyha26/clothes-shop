 <div class="rounded-lg border border-gray-200 bg-white p-6 shadow-sm ">
            ...
            <a href="/clothe/show/{{$clothe->id}}">
                <img class="mx-auto h-full" src="{{asset('/uploads/products/'.$clothe->photo)}}" alt="" />
            </a>
            </div>
            <div class="pt-6">
            <div class="mb-4 flex items-center justify-between gap-4">
                ...
                <button type="button" data-tooltip-target="tooltip-quick-look" class="rounded-lg p-2 text-gray-500 hover:bg-gray-100">
                ...
                <button type="button" data-tooltip-target="tooltip-add-to-favorites" class="rounded-lg p-2 text-gray-500 hover:bg-gray-100 hover:text-gray-900 ">
                    ...
                    @if(in_array($clothe->id, $favoriteProductIds))
                    <a href="/remove-favorite/{{$clothe->id}}">
                        <i class="fa-solid fa-heart text-red-400"></i>
                    </a>
                    @else
                    <a href="/add-favorite/{{$clothe->id}}">
                        <svg class="h-5 w-5 text-re" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
            ...
            <a href="/clothe/show/{{$clothe->id}}" class="text-lg font-semibold leading-tight text-gray-900 hover:underline">{{$clothe->name}}</a>
            ...
            <div class="mt-4 flex items-center justify-between gap-4 relative">
                <p class="text-2xl font-extrabold leading-tight text-gray-900 ">${{$clothe->price}}</p>
                @if (empty($clothe->quantity_in_stock))
                    <span class="text-white absolute bg-rose-500 p-2 bottom-0 right-0">Out Stock</span>
                @endif
